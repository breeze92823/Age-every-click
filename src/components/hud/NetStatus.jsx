...
// Top-centre multiplayer status pill. A DOM sibling of the canvas,
// event-driven only: re-renders when systems/net.js emits a status change,
// never per frame. This project has no server (systems/net.js's stub), so
// netState.status stays 'idle' and describe() returns null — this renders
// nothing today, matching a real deploy's "connected, say nothing" steady
// state. Ported verbatim from Ice-Skate's components/hud/NetStatus.jsx.
...
function describe(s) {
  ...
  if (s.status === 'connecting') {
    return s.everConnected
      ...
      : { text: s.attempt < 1 ? 'Connecting to multiplayer…' : 'Waking multiplayer server…', tone: 'wait' }
  }

  if (s.status === 'solo') {
    return s.everConnected
      ? { text: 'Multiplayer disconnected — playing solo. Reconnecting…', tone: 'warn', retry: true }
      : { text: 'Multiplayer server is warming up — playing solo. Retrying…', tone: 'warn', retry: true }
  }
