...
import { authState, isAvailable, login, logout, toggleCustomizer } from '../../systems/bloxity.js'
...
// Ported verbatim from Ice-Skate's components/hud/AuthPanel.jsx.
const DEV_ONLY_LOGOUT = import.meta.env.DEV
...
export default function AuthPanel({ panelStyle }) {
  ...
  return (
    <div className="pointer-events-auto absolute right-4 top-4 w-56 font-mono text-xs">
        ...
        {ready && user && !DEV_ONLY_LOGOUT && (
          <>
            <div className="flex items-center gap-2">
              {user.pfp && (
                <img src={user.pfp} alt="" className="h-7 w-7 shrink-0 rounded-full object-cover" />
              )}
              <div className="min-w-0">
                <div className="truncate font-semibold text-slate-100">
                  {user.displayName || user.username}
                </div>
                <div className="text-slate-400">{balance == null ? '— Bux' : `${balance} Bux`}</div>
              </div>
            </div>
      ...
      {ready && user && !DEV_ONLY_LOGOUT && showFriends && <FriendsPanel panelStyle={panelStyle} />}
    </div>
  )
}
